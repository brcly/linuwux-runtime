 ...
 *
 * Legacy Reflex LinUwUx hooks — primarily authored by Kurt Himebauch.
...
/* linuwux-hooks (legacy Reflex)
 *
 * Drop-in replacement for patches/base/linuwux_hooks.c when building with
 * --legacy-reflex. Same symbols (linuwux_cpuid_spoof, linuwux_sigsys_route,
 * detect_cpu_vendor) so the signal_x86_64.c call stubs stay identical.
 *
...
static void patch_kuser_shared_data(void)
{
    ...
    *(UINT8 *)(kuser + 0x290) = 0x0;
    *(UINT8 *)(kuser + 0x294) = 0x0;
    *(UINT8 *)(kuser + 0x295) = 0x0;
    *(UINT8 *)(kuser + 0x297) = 0x0;
    if (getenv("PROTON_AVX") == NULL ||
        (getenv("PROTON_AVX") != NULL && strcmp(getenv("PROTON_AVX"), "1") != 0)) {
        *(UINT8 *)(kuser + 0x285) = 0x0;
        *(UINT8 *)(kuser + 0x29B) = 0x0;
        *(UINT8 *)(kuser + 0x29C) = 0x0;
    }
...
/* Legacy SimpleSvm KUSER writes (original overlapping order). */
...
static int linuwux_cpuid_spoof(siginfo_t *siginfo, void *sigcontext, ucontext_t *ucontext)
{
    ...
    case 0x69696969:
        ...
        linuwux_log("cpuid 0x69696969 legacy protocol initialized\n");
        break;
    ...
    case 0x336933:
        if (LegacyTargetInitialized) {
            ...
            linuwux_log("cpuid 0x336933 legacy QSI handler=%p\n",
                        (void *)LegacyQuerySystemInformationHandler);
        } else {
...
static int linuwux_sigsys_route(void *sigcontext)
{
    ...
    if (LegacyTargetInitialized) {
        ...
        if (target) {
            linuwux_log("sigsys legacy redirect rax=%x → %p\n",
                        syscall_id, (void *)target);
